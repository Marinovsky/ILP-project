...
function Build_Associated_Arcs(T::TriggerArcTSP)
	AssoArcs = Vector{Vector{IntType}}(undef, T.NArcs)
	...
	for r in 1:T.NTriggers
		append!(AssoArcs[T.Trigger[r].target_arc_id], r)
	end
...
function Print_Associated_Arcs(T::TriggerArcTSP, Associated_Arcs::Vector{Vector{IntType}}, ListName::String)
	PrintHeader(ListName)
	for a in 1:T.NArcs
		print("[(",T.Arc[a].u,", ",T.Arc[a].v,")]: ")
		for r in Associated_Arcs[a]
			print("(",T.Arc[T.Trigger[r].trigger_arc_id].u," , ",T.Arc[T.Trigger[r].trigger_arc_id].v,") - (",T.Arc[T.Trigger[r].target_arc_id].u," , ",T.Arc[T.Trigger[r].target_arc_id].v,") ")
		end
		println()
	end
end
