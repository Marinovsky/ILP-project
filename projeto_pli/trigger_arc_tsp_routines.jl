...
function TriggerArcTSP_lb_rlxlag(T::TriggerArcTSP, MaxTime::IntType)
	...
	for k=1:MAX_ITER
		...
		# Stop when Z(lambda) in iteration k equals the previous Z(lambda) in iteration k-1
...
# --------------------------------------------------------------
function TriggerArcTSP_ilp(T::TriggerArcTSP, MaxTime::IntType)
	StartingTime = time()  # To compute the time used by this routine.
